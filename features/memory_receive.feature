Feature: Memory stability of lidi-receive under congestion
  ...
  # Issue 2 — lib.rs:281  to_decode   crossbeam_channel::unbounded() ........... T-SR9/T-SR9b
  # Issue 3 — lib.rs:282  to_dispatch crossbeam_channel::unbounded() ........... T-SR10/T-SR10b
  # Issue 4 — lib.rs:283  to_clients  crossbeam_channel::unbounded() ........... T-SR11/T-SR11b
  # Issue 5 — dispatch.rs:108  client_sendq unbounded when queue_size=0 ........ T-SR4/T-SR11/T-SR11b
  #
  # Note: Issues 4 and 5 share the same root cause and fix (client_queue_size > 0).
  # T-SR4 tests Issue 5 via SIGSTOP (Prometheus counter); T-SR11 tests it via CPU starvation.
  # Global pipeline tests (Issues 1-4): CPU starvation via taskset + chrt SCHED_IDLE + CPU hog.
  ...
  Scenario: T-SR1 - Receiver pipeline queues are observable during transfer
    ...
    And the receiver Prometheus gauge lidi_receive_decode_queue_len is greater than or equal to 0
  ...
  Scenario: T-SR2 - Pipeline queues are empty after transfer
    ...
    And the receiver Prometheus gauge lidi_receive_decode_queue_len is less than or equal to 0
  ...
  Scenario: T-SR8 - to_reblock grows unbounded when reblock thread is slow
    ...
    And wait 2 seconds
    And lidi-receive reblock thread is paused for 5 seconds
    Then receiver memory grew by more than 5 MB during thread pause
  ...
  Scenario: T-SR8b - reblock_queue_size=4 bounds memory when reblock thread is slow
    ...
    And decode_queue_size is configured to 128
  ...
  Scenario: T-SR9 - to_decode grows unbounded when decode thread is slow
    # Issue 2: to_decode is crossbeam_channel::unbounded() (decode_queue_size=0).
    # Reblock keeps producing complete blocks; when decode is starved the queue
    # fills without limit and lidi-receive RSS grows uncontrollably.
    # TDD test: demonstrates the bug by verifying memory DOES grow >5 MB.
    # Fix: set decode_queue_size > 0 (see T-SR9b).
    Given lidi is started with max throughput of 100mbit
    When lidi-file-send starts sending file tsr9.bin of size 100MB
    And wait 2 seconds
    And lidi-receive decode thread is paused for 5 seconds
    Then receiver memory grew by more than 5 MB during thread pause

  @wip
  Scenario: T-SR9b - decode_queue_size=4 bounds memory when decode thread is slow
    # Fix verified: decode_queue_size=4 caps to_decode at 4 Reassembled blocks.
    # Each Reassembled block ≈ block_size (220 KB), so 4 items ≈ 880 KB from to_decode.
    # reblock_queue_size=4 bounds to_reblock at 4 mmsg batches (≤ 4×1024×MTU ≈ 6 MB).
    # Cascade: decode paused → reblock blocks on to_decode → UDP workers block on to_reblock.
    Given lidi is started with max throughput of 100mbit
    And reblock_queue_size is configured to 4
    And decode_queue_size is configured to 4
    And dispatch_queue_size is configured to 128
    And clients_queue_size is configured to 128
    And lidi-receive is restarted
    When lidi-file-send starts sending file tsr9b.bin of size 100MB
    And wait 2 seconds
    And lidi-receive decode thread is paused for 5 seconds
    Then receiver memory did not grow by more than 10 MB during thread pause
  ...
  Scenario: T-SR10b - dispatch_queue_size=4 bounds memory when dispatch thread is slow
    ...
    # Cascade: dispatch paused → decode blocks on to_dispatch → reblock blocks on to_decode
    # → UDP workers block on to_reblock (4 mmsg batches ≤ 6 MB worst case).
    # All three upstream queues set to 4 to cap each stage of the cascade.
    Given lidi is started with max throughput of 100mbit
    And reblock_queue_size is configured to 4
    And decode_queue_size is configured to 4
  ...
  Scenario: T-SR11b - client_queue_size=4 bounds memory when client worker is slow
    ...
    # The upstream pipeline (reblock→decode→dispatch) runs normally; no cascade backpressure.
